package rewardCalculation.services.jobType;
...
@Slf4j
public class GetJobTypeService {

    private final JobTypeEntityRepository repository;
    private final GetJobTypeRequestValidator validator;
    ...
    public CommonResponseForJobTypeParameters execute(CommonRequestForJobTypeParameters request){
        ...
        if (errors.isEmpty()){
            JobTypeEntity jobType = repository.findById(request.getJobTypeDTO().getId()).get();
